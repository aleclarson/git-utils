...
var MergeStrategy, Promise, Random, assertValid, crawl, fs, git, log, optionTypes, path, sync;
  ...
  return git.isClean(modulePath).assert("The current branch cannot have any uncommitted changes!").then(function() {
    ...
    return Promise.chain(renamedPaths, function(newPath, oldPath) {
      ...
      return Promise.chain(unlinkedPaths, function(filePath) {
        ...
        return git.remove(modulePath, ourFile);
      });
    }).then(function() {
...
crawl = function(dir, files) {
  fs.readDir(dir).forEach(function(file) {
    file = path.join(dir, file);
    if (fs.isDir(file)) {
      return files.push(file);
    } else {
      return crawl(file, files);
    }
  });
  return files;
};
